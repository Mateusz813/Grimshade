...
/** Combat elixirs that always drain (no tier rules). */
...
/** Tier-grouped buffs — only the HIGHEST active tier drains per tick.
 *  Per spec the player can hold all 3 ATK elixir tiers in their bag and
 *  fire them simultaneously; the +100% buff will cover the swing first,
 *  and only its time will tick down. When +100% expires, +50% takes over
 *  next tick, then +25% — preserving "no stacking, top tier first". */
...
export const tickCombatElixirs = (ms: number): void => {
    ...
    // Tier-grouped — drain only the highest active tier so lower tiers
    // preserve their full duration until the higher one runs out.
    for (const group of [ATK_TIERS_HIGH_FIRST, SPELL_TIERS_HIGH_FIRST]) {
        for (const effect of group) {
            if (b.hasBuff(effect)) {
                b.consumePausableTime(effect, ms);
                break; // only the first hit (= highest tier) drains
            }
        }
    }
};
