 ...
 *  1. Default state — `Zwykle` button bez --active + tekst `Zwykle ✗`.
 *  2. Tap "Zwykle" → button dostaje klasę `--active` + tekst `Zwykle ✓`.
    ...
    test('toggle Common auto-sell → button flips active class + text bidirectionally', async ({ page }) => {
        ...
        try {
            ...
            for (let attempt = 1; attempt <= 3; attempt++) {
                ...
                try {
                    await expect(commonBtn).toContainText('Zwykle ✓', { timeout: 3_000 });
                    break;
                } catch (err) {
            ...
            await expect(rareBtn).toContainText('Rzadkie ✗');
