 ...
 *  1. Tap ":hammer: Rozloz" toggle na header-ze plecaka -> bulkMode='disassemble'.
 *  2. Tap "Zaznacz wszystkie" -> wszystkie 3 seeded items oznaczone.
 *  3. Stopka pokazuje ":hammer: Rozloz zaznaczone (3 szt.)" -> tap.
 *  4. ~1.2s animacja progress -> wszystkie items znikają + popup
 *     ":hammer: Rozkladanie zakonczone!" z liczbą rozłożonych przedmiotów +
 *     listą kamieni.
 *
 ...
 * Asercje:
 *  - Przed: 3 bag tiles.
 *  - Tap ":hammer: Rozloz" toggle -> bulkMode-label ":hammer: Tryb rozkladania" widoczna.
 *  - Tap "Zaznacz wszystkie" -> 3 tiles z klasą --selected.
 *  - Stopka ":hammer: Rozloz zaznaczone (3 szt.)" widoczna.
...
test.describe('Inventory › Disassemble', { tag: '@inventory' }, () => {
    ...
    test('bulk mode -> select all -> "Rozloz zaznaczone" -> all removed + result popup with 3 stones (stubbed RNG)', async ({ page }) => {
        ...
        try {
            ...
            // 6. Tap ":hammer: Rozloz" toggle (multi-sell-toggle--disassemble) —
            ...
            // 7. Bulk mode label ":hammer: Tryb rozkladania" widoczna.
            ...
            // 9. Stopka ":hammer: Rozloz zaznaczone (3 szt.)" pojawia się
